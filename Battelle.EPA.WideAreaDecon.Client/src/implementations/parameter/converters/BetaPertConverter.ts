...
export default class BetaPertConverter implements IParameterConverter {
  // eslint-disable-next-line class-methods-use-this
  convertToNewType(old: IParameter, newType: ParameterType): IParameter {
    if (old.type !== ParameterType.pert) {
      throw new Error('Beta PERT converter only works with Beta PERT parameter');
    }

    const oldParam = <BetaPERT>old;
    ...
    switch (newType) {
      ...
      case ParameterType.pert:
        return new BetaPERT(old.metaData, oldParam.min, oldParam.max, oldParam.mode);
