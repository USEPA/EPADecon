...
export default class BimodalTruncatedNormalConverter implements IParameterConverter {
  // eslint-disable-next-line class-methods-use-this
  convertToNewType(old: IParameter, newType: ParameterType): IParameter {
    if (old.type !== ParameterType.bimodalTruncatedNormal) {
      throw new Error('Bimodal Truncated Normal converter only works with Bimodal Truncated Normal parameter');
    }

    const oldParam = <BimodalTruncatedNormal>old;
